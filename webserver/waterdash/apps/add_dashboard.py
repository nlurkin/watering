...
def get_layout(update):
    if update:
        field_to_use = [dbc.Input(id = "add_dashboard_name", placeholder = "Dashboard name", type = "text", pattern = "[a-zA-Z0-9_]+", style = {"display": "None"}),
                        dcc.Dropdown(id = "update_dashboard_name", placeholder = "Dashboard name", options = mongoClient.get_dashboards_dropdown()), ]
    else:
        field_to_use = [dbc.Input(id = "add_dashboard_name", placeholder = "Dashboard name", type = "text", pattern = "[a-zA-Z0-9_]+"),
                        dcc.Dropdown(id = "update_dashboard_name", placeholder = "Dashboard name", style = {"display": "None"}), ]
    ...
    return [html.Div(
        ...
        dbc.Modal([
            ...
            dbc.ModalFooter(
                dbc.Button("Close", id = "update_dashboard_modal_close", className = "ml-auto")
                ),
            ],
...
# Callbacks
# ========================
@app.callback([Output('update_dashboard_modal', 'is_open'), Output('update_dashboard_modal_message', 'className'), Output('update_dashboard_modal_message', 'children'),
               Output({"type": "group_cards", "index": "left"}, "children"), Output({"type": "group_cards", "index": "right"}, "children")],
              [Input({"type": "add_element", "index": "left"}, 'n_clicks'), Input({"type": "add_element", "index": "right"}, 'n_clicks'),
               Input("update_dashboard_name", "value")],
              [State({"type": "group_cards", "index": "left"}, "children"), State({"type": "group_cards", "index": "right"}, "children")])
def update_layout(btn_add_left, btn_add_right, update_name, left_col, right_col):
    ...
    if "add_element" in trig_id:
        ...
        if trig_id["index"] == "left":
            return False, "", "", add_element_col(None, left_col, "left"), dash.no_update
        if trig_id["index"] == "right":
            return False, "", "", dash.no_update, add_element_col(None, right_col, "right")
...
@app.callback([Output('add_dashboard_modal', 'is_open'), Output('add_dashboard_modal_message', 'className'), Output('add_dashboard_modal_message', 'children')],
              ...
              [State("add_dashboard_name", "value"), State("add_dashboard_name", "pattern"),
...
def save_dashboard(submit, close, dashboard_name, dashboard_name_pattern, update_id, sensors, sensors_ids, is_open):
    ctx = dash.callback_context
    ...
    if dashboard_name is None and update_id is not None:
        ...
        return add_dashboard(dashboard_name, dashboard_name_pattern, left_sensors, right_sensors)
...
def build_layout(dashboard_name):
    ...
    if dashboard_doc is None:
        return True, "message-error", f"Unable to find sensor id {dashboard_name}", dash.no_update, dash.no_update
...
def update_dashboard(dashboard_id, left_sensors, right_sensors):
    ...
    return True, "message-valid", f"Dashboard with ID:{dashboard_id} successfully updated"
