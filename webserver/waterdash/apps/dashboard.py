                ...
                dbc.Checklist(
                    ...
                    value=(
                        ...
                        if (
                            ...
                            or controller_last_value[-1]["val"] == 0
                        )
                        else [1]
                    ),
                    id={"type": "bool_controller", "sensor": sensor["sensor"]},
                    switch=True,
                )
            )
            sensor_element.append(
                html.P(id={"type": "dummy", "sensor": sensor["sensor"]})
            )
...
def build_controls():
    ...
    return dbc.Collapse(
        dbc.Row(
            [
                dbc.Col(switches, width=1),
                dbc.Col(date_range, width=2),
                dbc.Col(update_interval, width=1),
            ]
        ),
        id="control_menu",
    )
...
def generate_layout(dashboard_name):
    ...
    return [
        ...
        dbc.Row(
            [
                dbc.Col(left_col, style={"padding": "0px"}),
                dbc.Col(right_col, style={"padding": "0px"}),
            ]
        ),
    ]
...
@app.callback(Output("interval-component", "interval"), Input("refresh_rate", "value"))
...
@app.callback(
    Output({"type": "string_sensor", "sensor": MATCH}, "value"),
...
@app.callback(
    Output({"type": "bool_sensor", "sensor": MATCH}, "figure"),
    [Input("interval-component", "n_intervals")],
    [
...
def update_bool_metrics(_, sensor_name, control_switches, sd, ed):
    df, sensor_doc, min_max, with_setpoint = get_and_merge_data(
        sensor_name["sensor"], sd, ed
    )
...
@app.callback(
    Output({"type": "dummy", "sensor": MATCH}, "value"),
...
@app.callback(
    Output({"type": "float_sensor", "sensor": MATCH}, "figure"),
    [Input("interval-component", "n_intervals")],
    [
...
def set_style(figure, title, control_switches, data_range):
    ...
    if 1 in control_switches:
        ...
        init_range = [
            ...
            now + datetime.timedelta(minutes=offset_minutes),
        ]
        xaxis.update(
            {
    ...
    xaxis["rangeselector"] = dict(
        buttons=list(
            [
                ...
                dict(
                    count=30 + offset_minutes,
                    label="30m",
                    step="minute",
                    stepmode="todate",
                ),
                dict(
                    count=5 + offset_minutes,
                    label="5m",
                    step="minute",
                    stepmode="todate",
                ),
