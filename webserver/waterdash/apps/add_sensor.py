    ...
    if update:
        field_to_use = [
            ...
            dcc.Dropdown(
                ...
                placeholder="Sensor name",
                options=mongoClient.get_sensors_dropdown(),
            ),
        ]
        button_text = "Update"
    else:
    ...
    return [
            ...
            [
                ...
                dbc.FormGroup(
                    [
                        dbc.Label(
                            "Sensor display name",
                            html_for="add_sensor_display",
                            width=2,
                        ),
                        dbc.Col(
                            dbc.Input(
                                id="add_sensor_display",
                ...
                dbc.FormGroup(
                    [
                        ...
                        dbc.Col(
                            dbc.Checklist(
                                id="add_sensor_controller",
                                options=[
                                    {"label": "Is controller", "value": 1},
                                ],
                                switch=True,
                            ),
                            width=2,
                ...
                dbc.FormGroup(
                    [
                        ...
                        dbc.Col(
                            dbc.Button(
                                button_text, id="add_sensor_submit", color="primary"
                            ),
                            width=2,
                        ),
                    ],
        ...
        dbc.Modal(
            [
                ...
                dbc.ModalFooter(
                    dbc.Button(
                        "Close", id="add_sensor_modal_close", className="ml-auto"
                    )
                ),
            ],
            id="add_sensor_modal",
        ...
        dbc.Modal(
            [
                ...
                dbc.ModalFooter(
                    dbc.Button(
                        "Close", id="update_sensor_modal_close", className="ml-auto"
                    )
                ),
            ],
...
@app.callback(
    [
        Output("update_sensor_modal", "is_open"),
        Output("update_sensor_modal_message", "className"),
        Output("update_sensor_modal_message", "children"),
        Output("add_sensor_display", "value"),
        Output("add_sensor_type", "value"),
        Output("add_sensor_controller", "value"),
    ],
...
def update_display(sensor_name):
    ...
    if sensor_doc is None:
        return (
            True,
            "message-error",
            f"Unable to find sensor id {sensor_name}",
            dash.no_update,
            dash.no_update,
        )
    ...
    return (
        ...
        "",
        sensor_doc["display"],
        sensor_doc["data-type"],
        [1] if ("controller" in sensor_doc and sensor_doc["controller"]) else [],
    )


@app.callback(
    [
        Output("add_sensor_modal", "is_open"),
        Output("add_sensor_modal_message", "className"),
        Output("add_sensor_modal_message", "children"),
    ],
    [
        Input("add_sensor_submit", "n_clicks"),
        Input("add_sensor_modal_close", "n_clicks"),
    ],
    [
    ...
    if name is not None:
        ...
        if name is None or stype is None or not re.match(name_pattern, name):
            return (
                True,
                "message-error",
                "Sensor name is invalid, only alphanumerical characters and _ are allowed",
            )
        if display is None:
            display = name
        ...
        data_dict = {
            ...
            "display": display,
            "data-type": stype,
            "controller": 1 if is_controller is not None and 1 in is_controller else 0,
        }
        ...
        if sensor_doc is not None:
            return (
                True,
                "message-error",
                f"A sensor with same name already exists. Choose another name.",
            )
        ...
        data_dict = {
            "display": display,
            "data-type": stype,
            "controller": 1 in is_controller,
        }
        mongoClient.update_sensor_by_id(update_name, data_dict)
        return (
            True,
            "message-valid",
            f"Sensor with ID {update_name} successfully updated",
        )
