...
layout = [html.Div(
        [
            dbc.FormGroup([
                ...
                dbc.Col(dbc.Input(id = "add_sensor_name", placeholder = "Sensor name", type = "text", pattern = "[a-zA-Z0-9_]+"), width = 2),
                ], row = True),
            dbc.FormGroup([
...
def update_output(submit, close, name, name_pattern, display, stype, is_open):
    ...
    sensor_doc = mongoClient.sensors_db.find_one({"sensor": name})
    if sensor_doc is not None:
        return True, "message-error", f"A sensor with same name already exists. Choose another name."

    sensor_doc = mongoClient.sensors_db.insert_one(data_dict)
    return True, "message-valid", f"Sensor successfully added with ID:{sensor_doc.inserted_id}"
