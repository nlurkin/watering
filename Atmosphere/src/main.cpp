...
bool watchdog()
{
  ...
  { // Indicate connection lost
    ...
    { // Double check, maybe only IP lost and recovered afterward
      _m_welcome.connected(false);
      connected = false;
      // Seems we really lost the wifi...
      watchdog_ok = false;
      if (reset_failure > 10)
      { // Wifi reconnect failed many times. Try a full RESET of the ESP
      ...
      { // No RESET at this point, just try to reconnect
        char p_ssid[30], p_pwd[30];
        strcpy_P(p_ssid, ssid);
        strcpy_P(p_pwd, pwd);
